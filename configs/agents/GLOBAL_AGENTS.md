...
- Atlas (`~/Documents/Kosmos/Atlas`) is the LLM-maintained wiki and evolving map over Terra and Forge. For every user prompt, use hook-injected Evidence as a broad first pass and attempt one deliberate Kosmos MCP Query (`mcp__kosmos__query`); read or expand only what is useful. If the MCP is unavailable, continue with the best available fallback and say so.
