...
- Read the miss, then escalate deliberately: an Atlas hit is a cache hit (load details over its links); Atlas empty but Terra/Forge rich means answer from substrate and compile the reusable core into `10_wiki/`; everything empty on a question that matters means bounded web research (fan-out & synthesize in the chat), then compile the finding back into Atlas. Escalation to the web is chat judgment — the query service itself never fetches.
