...
- Read the miss, then escalate deliberately — the query flags it itself via `coverage_signal`: `atlas_empty_territory_support` means answer from Terra/Forge substrate and compile the reusable core into `10_wiki/`; `possible_gap` on a question that matters means bounded web research (fan-out & synthesize in the chat), then compile the finding back into Atlas. A weak retrieval score alone is not a knowledge gap — escalate on concrete need (task-blocking miss, unsupported fact, contradiction), and the query service itself never fetches.
