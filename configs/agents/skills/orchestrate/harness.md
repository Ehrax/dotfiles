...
- Full flag details and traps: the `codex` skill.
...
- Using gpt-5.5 inside workflows/subagents (the `model` parameter only takes Claude models, so wrap it — pattern verified end-to-end):
  ...
  - The wrapper must BLOCK until codex exits — a wrapper that backgrounds codex and ends its turn gets finalized, which kills the codex child mid-run (see the `codex` skill).
  - Always prefix the label with `gpt-5.5:` (e.g. `{label: 'gpt-5.5:review-auth'}`) — the UI shows the wrapper's Claude model, so the label is the only signal who really did the work.
  ...
  - Parallel gpt-5.5 implementers need `isolation: 'worktree'` so codex edits don't collide in the shared checkout.
  - Workflow token budgets count only Claude tokens; codex work is invisible to `budget.spent()`.
- Model choice per role: the project AGENTS.md model table ("Picking models for delegated work").
