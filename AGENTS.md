...
| `configs/Codex/settings.json` | `~/.Codex/settings.json` AND `~/.Codex-work/settings.json` |
| `configs/Codex/mcp.json` | `~/.Codex/mcp.json` AND `~/.Codex-work/mcp.json` |
| `configs/Codex/statusline.sh` | `~/.Codex/statusline.sh` |
| `configs/agents/skills/` | `~/.Codex/skills/`, `~/.Codex-work/skills/`, `~/.config/opencode/skills/`, `~/.gemini/skills/`, AND `~/.codex/skills/` |
| `configs/opencode/opencode.json` | `~/.config/opencode/opencode.json` |
| `configs/tmux/gitmux.conf` | `~/.config/tmux/gitmux.conf` |

### Dual Codex Config

Two Codex profiles exist, switchable via `cc` (personal → `~/.Codex`) and `cm` (work → `~/.Codex-work`). Both share the same `settings.json`, `mcp.json`, skills, and plugins from this repo, and OpenCode shares the same skills source. The active profile is persisted via `CLAUDE_CONFIG_DIR` in `~/.fish_claude_dir` and restored on shell startup.
...
Consistent One Dark colorscheme (`bg: #21252b`) across Ghostty, Neovim, tmux, lazygit, and the Codex statusline. When adding or modifying terminal/UI config, keep this palette.
...
- `c` — launch Codex (opus, dangerously-skip-permissions)
- `cw` — Codex in a worktree with tmux
- `cwl` — list git worktrees
- `cwd <path>` — remove a git worktree and prune
- `cc` / `cm` — switch Codex config to personal / work
- `lg` — lazygit
- `v` / `vim` — nvim
...
- Ghostty uses `term = xterm-256color` (not `xterm-ghostty`) for compatibility with CLI tools like Codex that don't recognize the Ghostty terminfo.
